...
from engine import PhyslibSearchEngine, QueryResult
...
def main(query: list[str], num_results: int, json_output: bool):
    with psycopg.connect(os.environ["CONNECTION_STRING"], autocommit=True) as conn:
        retriever = PhyslibSearchEngine(os.environ["CHROMA_PATH"], conn)
        results = retriever.find_declarations(query, num_results)
        if json_output:
