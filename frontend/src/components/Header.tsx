...
export default function Header({ currentPath }: Props) {
  return (
    <header className="fixed top-4 left-1/2 -translate-x-1/2 z-50 flex items-center gap-1.5 md:gap-3 px-2.5 md:px-5 py-2 md:py-2.5 rounded-full bg-background/80 backdrop-blur-lg border border-foreground/10 shadow-lg shadow-foreground/5 max-w-[calc(100vw-1rem)]">
      {/* Brand */}
      <Link
        href="/"
      ...
      >
        <span className="font-bold text-sm md:text-base tracking-tight whitespace-nowrap">
          <span className="hidden sm:inline">PhyslibSearch</span>
          <span className="sm:hidden">Physlib</span>
        </span>
        <span className="font-light text-xs text-foreground/35 hidden lg:inline">by</span>
        {/* eslint-disable-next-line @next/next/no-img-element */}
        <img
          src="/KS_logo.png"
          alt="Kernel Science"
          style={{ height: "16px", width: "auto" }}
          className="dark:invert opacity-65 hidden lg:block"
        />
      </Link>
